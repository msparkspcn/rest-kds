...
const OrderHeader: React.FC<OrderHeaderProps> = ({ orderNo, instTime, diff }) => (
...
const RenderItem: React.FC<RenderItemProps> = ({ item, index }) => (
    <button className="w-full h-9 text-black" onClick={() => {}}>
        <div className="flex w-full">
            <div className="flex-1 border border-gray-400 bg-white-500 p-1">{index+1}</div>
            <div className="flex-8 border border-gray-400 bg-white-500 p-1 flex justify-start items-end">{item.productNm}</div>
            <div className="flex-1 border border-gray-400 bg-white-500 p-1">{item.saleQty}</div>
        </div>
    </button>
);
